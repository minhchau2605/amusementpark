<?php
	...
	//List Detail Booking
	/*$sql = "SELECT booking.booking_id,user_name,booking_date,ticket_date,total,booking_status,zone_name,sub_total from booking 
	...
	where {$condition} ORDER BY booking_id DESC";

	$rs = $db->fetchAll($sql);

	foreach($rs as $row){
		$xp->insert_loop('LIST.LS.DETAIL',array('DETAIL'=>$row));
	}*/
